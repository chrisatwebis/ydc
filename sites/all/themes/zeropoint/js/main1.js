...
/* 	clearFormFields({
	...
	initCastomForms();
	//force repaint when the province field changes.
/* 	$("#edit-submitted-left-column-province").change(function(){	
			replaceSelects();	
	}); */
	
	//VSA_initScrollbars();
	$("#optionsDiv1 ul li").click(function(){
		//$("#views-exposed-form-taxonomy-term-gallery-term-views .views-submit-button input").trigger('click');
		$("#views-exposed-form-taxonomy-term-gallery-term-views").submit();
	});
}
...
function replaceSelects() {
	...
	if (!selects[q].replaced && selects[q].offsetWidth && selects[q].className.indexOf("default") == -1)
	{
		...
		var stWidth = parseInt(selects[q].offsetWidth) + 20; // Just using the offsetWidth was too narrow in IE8.
...
function selectMe(selectFieldId,linkNo,selectNo) {
	...
	for(var k = 0; k < selectField.options.length; k++) {
		if(k==linkNo) {
			...
			if(selectFieldId == 'edit-submitted-left-column-age' || selectFieldId == 'edit-submitted-left-column-province' ){			
				$("#"+selectFieldId).change();
			}
			if( selectFieldId == 'edit-submitted-left-column-province' &&  $("#"+selectFieldId).val() != ''){			
	...
	if(selectSponsor == false)
		selectSponsor = true;
	else {
		if(selectField.className === "sponsorship outtaHere")
			window.open(selectField.value, "_blank");
	}
}
//showing options
...
var VSA_default_imagesPath = "/sites/bmo.com/themes/bmo_soccer/images";
...
function captureEvent(actionValue) {
        var lc=new Image();
        lc.src = "https://csvtr.bmo.com/bmo/zag2.gif?Log=1&action="+actionValue+"&cb="+new Date().getTime();}

function setup_search_input_val(input) {
	var val_search;
	$(input).click(function () {
	val_search = $(input).val();
	if(val_search == 'Name, City, Province' || val_search == 'Nom, ville, province') {
		$(input).val('');
	}
	});
	...
	$(input).blur(function () {
		...
		if(val_search.length == 0) {
			var langlinks;
			langlinks = $("#langlinks li.last a").text();
			if(langlinks == "English") {
				$(input).val('Nom, ville, province');
			} else {
				$(input).val('Name, City, Province');
			}
		}
	});
}
...
$(document).ready(function() {
	setup_search_input_val("form.search-form input");
	setup_search_input_val("form.keyword-search input");
	
	// search
	$("body.node-type-soccer_team form.search-form a.sg-button em").click(function () {
		$("body.node-type-soccer_team form.search-form input.btn-search").click();
	});
